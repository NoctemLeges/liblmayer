...
void drawLSystem(Word *instructions, double angleDelta, double step, const char *filename, int isTree)
{
    ...
    if(isTree){
        ...
        for (size_t i = 0; i < instructions->currSize; i++)
        {
            ...
            if(c=='[') branchNo++;
            if(c==']') branchNo--;
            if (isalpha(c) && branchNo<THRESHOLD)
            {
                double newX = x + step * cos(angle * M_PI / 180.0);
                double newY = y + step * sin(angle * M_PI / 180.0);
                fprintf(svg, "<line stroke=\"brown\" stroke-width=\"3\" x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" />\n",
                        x + offsetX, height - (y + offsetY),
                        newX + offsetX, height - (newY + offsetY));
                x = newX;
                y = newY;
            }
            else if (isalpha(c) && branchNo>=THRESHOLD)
            {
                double newX = x + step * cos(angle * M_PI / 180.0);
                double newY = y + step * sin(angle * M_PI / 180.0);
                fprintf(svg, "<line stroke=\"green\" x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" />\n",
                        x + offsetX, height - (y + offsetY),
                        newX + offsetX, height - (newY + offsetY));
                x = newX;
                y = newY;
